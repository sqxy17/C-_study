...
//3.注意事项
//使用auto必须在使用的时候初始化            -----也就是=右边必须有变量或者一个值，否则不知道如何推导
//不能用来定义数组  int arr1[];auto arr2[]//后面的这样定义数组时错误的用法            ----不能用作推导数组
...
//{
//    int arr[]={1,2,3,4,5,6,7,8};
//    for(int& llk:arr)//可理解别名的生命周期是一次性的，这样就可以操作元素了          -----arr[i]这就是右值的一种，可以引用，通过引用操作里面的元素
//    {
//        llk++;
//    }
//
